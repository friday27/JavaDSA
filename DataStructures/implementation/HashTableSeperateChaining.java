...
public class HashTableSeperateChaining<K, V> implements Iterable<K> {
    ...
    public HashTableSeperateChaining(int capacity, double maxLoadFactor) {
        ...
        this.capacity = Math(DEFAULT_CAPACITY, capacity);
    ...
    //Strips the negative sign and places hash value in the domain [0, capacity)
    ...
    private V bucketSeekEntry(int bucketIndex, K key) {
        if(key == null)
            return null;
        LinkedList<Entry<K, V>> bucket = table[bucketIndex];
        if(bucket == null)
            return null;
        for(Entry<K, V> entry: buckey) {
    ...
    private void resizeTable() {
        ...
        for(int i=0; i<table.length; i+_) {
            if(table[i] != null) {
